...
export default function QuoteFormSection() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        setSuccessMsg(t("quoteSuccess"));
        setFormData({ name: "", mobile: "", email: "", requirement: "" });
      } else {
        setErrorMsg(t("quoteError"));
      }
    } catch (error) {
    ...
    <div className="Section bg-[#D9D9D9]/12" id="Enquiry">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
        ...
        <div>
          ...
          <p className="Paragraph">{t("quoteParagraph1")}</p>
          <p className="Paragraph">{t("quoteParagraph2")}</p>
        </div>
        ...
        <div className="bg-[#0E509E] text-white p-6 md:p-8 rounded-md shadow-md w-full max-w-md mx-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <textarea
              name="requirement"
              required
              placeholder={t("placeholderRequirement")}
